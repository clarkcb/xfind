            ...
            if (arg.charAt(0) === '-') {
                ...
                for (const argName of argNames) {
                    if (this.boolMap[argName]) {
                        argTokens.push(
                            new ArgToken(this.boolMap[argName], ArgTokenType.Bool, true)
                        );
                    } else if (
                        this.strMap[argName] ||
                        this.intMap[argName] ||
                        ...
                        if (args.length > 0) {
                            ...
                            if (this.strMap[argName]) {
                                argTokens.push(
                                    new ArgToken(this.strMap[argName], ArgTokenType.Str, argValue)
                                );
                                ...
                                argTokens.push(
                                    new ArgToken(
                                        this.intMap[argName],
                                        ArgTokenType.Int,
                                        parseInt(argValue, 10)
                                    )
                                );
                            } else {
                                argTokens.push(
                                    new ArgToken('settings-file', ArgTokenType.Str, argValue)
                                );
                            }
                        } else {
                ...
                if (argsObj[argName] !== undefined && argsObj[argName] !== null) {
                    ...
                    if (this.boolMap[argName]) {
                        ...
                        if (typeof argsObj[argName] === 'string') {
                            ...
                            typeof argsObj[argName] === 'object' &&
                            Array.isArray(argsObj[argName])
                        ) {
                            argsObj[argName].forEach((s) => {
                                if (typeof s === 'string') {
                                    argTokens.push(
                                        new ArgToken(this.strMap[argName], ArgTokenType.Str, s)
                                    );
                                } else {
                                    err = new FindError(`Invalid value for option: ${argName}`);
                                }
                        ...
                        argTokens.push(
                            new ArgToken('settings-file', ArgTokenType.Str, argsObj[argName])
                        );
                    } else {
