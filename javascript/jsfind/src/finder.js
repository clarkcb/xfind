...
class Finder {
  ...
  validateSettings() {
    try {
      ...
      if (this.settings.maxDepth > -1 && this.settings.minDepth > -1) {
        assert.ok(
          this.settings.maxDepth >= this.settings.minDepth,
          invalidRangeForMinDepthAndMaxDepth
        );
      }
      if (this.settings.maxLastMod > 0 && this.settings.minLastMod > 0) {
        assert.ok(
          this.settings.maxLastMod >= this.settings.minLastMod,
          invalidRangeForMinLastModAndMaxLastMod
        );
      }
      if (this.settings.maxSize > 0 && this.settings.minSize > 0) {
        assert.ok(this.settings.maxSize >= this.settings.minSize, invalidRangeForMinSizeAndMaxSize);
      }
    } catch (err) {
      let msg = err.message;
      if (err.code === ENOENT) {
  ...
  isMatchingArchiveExtension(ext) {
    return (
      Finder.emptyOrMatchesAnyElement(ext, this.settings.inArchiveExtensions) &&
  ...
  isMatchingArchiveFileName(fileName) {
    return (
      Finder.emptyOrMatchesAnyPattern(fileName, this.settings.inArchiveFilePatterns) &&
  ...
  isMatchingExtension(ext) {
    return (
      Finder.emptyOrMatchesAnyElement(ext, this.settings.inExtensions) &&
  ...
  isMatchingFileName(fileName) {
    return (
      Finder.emptyOrMatchesAnyPattern(fileName, this.settings.inFilePatterns) &&
  ...
  isMatchingFileType(fileType) {
    return (
      Finder.emptyOrMatchesAnyElement(fileType, this.settings.inFileTypes) &&
  ...
  isMatchingFileSize(fileSize) {
    return (
      (this.settings.maxSize === 0 || fileSize <= this.settings.maxSize) &&
  ...
  isMatchingLastMod(lastMod) {
    return (
      (this.settings.maxLastMod === 0 || lastMod <= this.settings.maxLastMod) &&
  ...
  async find() {
    ...
    const pathFileResultsArrays = await Promise.all(
      this.settings.paths.map((d) => this.getFileResults(d))
    );
