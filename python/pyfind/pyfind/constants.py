# -*- coding: utf-8 -*-
"""
###############################################################################
#
# constants.py
#
# Constant values
#
###############################################################################
"""

INVALID_RANGE_MINDEPTH_MAXDEPTH = 'Invalid range for mindepth and maxdepth'
INVALID_RANGE_MINLASTMOD_MAXLASTMOD = 'Invalid range for minlastmod and maxlastmod'
INVALID_RANGE_MINSIZE_MAXSIZE = 'Invalid range for minsize and maxsize'
START_PATH_NOT_DEFINED = 'Startpath not defined'
START_PATH_NOT_FOUND = 'Startpath not found'
START_PATH_NOT_READABLE = 'Startpath not readable'
START_PATH_DOES_NOT_MATCH_FIND_SETTINGS = 'Startpath does not match find settings'
