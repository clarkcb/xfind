...
sub new {
    my $class = shift;
    my $config = shift;
    my $options_hash = load_options_from_json_file($config->{find_options_path});
...
sub load_options_from_json_file {
    # $find_options_path is an instance of Path::Class::File
    my $find_options_path = shift;
    my $options_hash = {};
...
sub update_settings_from_default_files {
    my ($self, $settings) = @_;
    my @errs;
    if (-e $self->{config}->{default_find_settings_path}) {
        my $e = $self->update_settings_from_file($settings, $self->{config}->{default_find_settings_path});
        @errs = @$e;
    }
    return \@errs;
}
