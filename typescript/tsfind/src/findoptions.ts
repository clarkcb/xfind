...
export class FindOptions {
  ...
  constructor() {
    ...
    this.boolActionMap = {
      archivesonly: (b: boolean, settings: FindSettings) => {
        settings.archivesOnly = b;
      },
      colorize: (b: boolean, settings: FindSettings) => {
        settings.colorize = b;
      },
      debug: (b: boolean, settings: FindSettings) => {
        settings.debug = b;
      },
      defaultfiles: (b: boolean, settings: FindSettings) => {
        settings.defaultFiles = b;
      },
      excludearchives: (b: boolean, settings: FindSettings) => {
        settings.includeArchives = !b;
      },
      excludehidden: (b: boolean, settings: FindSettings) => {
        settings.includeHidden = !b;
      },
      followsymlinks: (b: boolean, settings: FindSettings) => {
        settings.followSymlinks = b;
      },
      includearchives: (b: boolean, settings: FindSettings) => {
        settings.includeArchives = b;
      },
      includehidden: (b: boolean, settings: FindSettings) => {
        settings.includeHidden = b;
      },
      help: (b: boolean, settings: FindSettings) => {
        settings.printUsage = b;
      },
      nocolorize: (b: boolean, settings: FindSettings) => {
        settings.colorize = !b;
      },
      nodefaultfiles: (b: boolean, settings: FindSettings) => {
        settings.defaultFiles = !b;
      },
      nofollowsymlinks: (b: boolean, settings: FindSettings) => {
        settings.followSymlinks = !b;
      },
      noprintdirs: (b: boolean, settings: FindSettings) => {
        settings.printDirs = !b;
      },
      noprintfiles: (b: boolean, settings: FindSettings) => {
        settings.printFiles = !b;
      },
      norecursive: (b: boolean, settings: FindSettings) => {
        settings.recursive = !b;
      },
      printdirs: (b: boolean, settings: FindSettings) => {
        settings.printDirs = b;
      },
      printfiles: (b: boolean, settings: FindSettings) => {
        settings.printFiles = b;
      },
      recursive: (b: boolean, settings: FindSettings) => {
        settings.recursive = b;
      },
      'sort-ascending': (b: boolean, settings: FindSettings) => {
        settings.sortDescending = !b;
      },
      'sort-caseinsensitive': (b: boolean, settings: FindSettings) => {
        settings.sortCaseInsensitive = b;
      },
      'sort-casesensitive': (b: boolean, settings: FindSettings) => {
        settings.sortCaseInsensitive = !b;
      },
      'sort-descending': (b: boolean, settings: FindSettings) => {
        settings.sortDescending = b;
      },
      verbose: (b: boolean, settings: FindSettings) => {
        settings.verbose = b;
      },
      version: (b: boolean, settings: FindSettings) => {
        settings.printVersion = b;
      },
    };

    this.stringActionMap = {
      'in-archiveext': (s: string, settings: FindSettings) => {
        settings.addInArchiveExtensions(s);
      },
      'in-archivefilepattern': (s: string, settings: FindSettings) => {
        settings.addInArchiveFilePatterns(s);
      },
      'in-dirpattern': (s: string, settings: FindSettings) => {
        settings.addInDirPatterns(s);
      },
      'in-ext': (s: string, settings: FindSettings) => {
        settings.addInExtensions(s);
      },
      'in-filepattern': (s: string, settings: FindSettings) => {
        settings.addInFilePatterns(s);
      },
      'in-filetype': (s: string, settings: FindSettings) => {
        settings.addInFileTypes(s);
      },
      maxlastmod: (s: string, settings: FindSettings) => {
        settings.maxLastModFromString(s);
      },
      minlastmod: (s: string, settings: FindSettings) => {
        settings.minLastModFromString(s);
      },
      'out-dirpattern': (s: string, settings: FindSettings) => {
        settings.addOutDirPatterns(s);
      },
      'out-archiveext': (s: string, settings: FindSettings) => {
        settings.addOutArchiveExtensions(s);
      },
      'out-archivefilepattern': (s: string, settings: FindSettings) => {
        settings.addOutArchiveFilePatterns(s);
      },
      'out-ext': (s: string, settings: FindSettings) => {
        settings.addOutExtensions(s);
      },
      'out-filepattern': (s: string, settings: FindSettings) => {
        settings.addOutFilePatterns(s);
      },
      'out-filetype': (s: string, settings: FindSettings) => {
        settings.addOutFileTypes(s);
      },
      path: (s: string, settings: FindSettings) => {
        settings.paths.push(s);
      },
      'sort-by': (s: string, settings: FindSettings) => {
        settings.sortBy = SortUtil.nameToSortBy(s);
      },
    };

    this.intActionMap = {
      maxdepth: (i: number, settings: FindSettings) => {
        settings.maxDepth = i;
      },
      maxsize: (i: number, settings: FindSettings) => {
        settings.maxSize = i;
      },
      mindepth: (i: number, settings: FindSettings) => {
        settings.minDepth = i;
      },
      minsize: (i: number, settings: FindSettings) => {
        settings.minSize = i;
      },
    };
  ...
  public settingsFromArgs(
    args: string[],
    cb: (err: Error | undefined, settings: FindSettings) => void,
  ): void {
