...
export class Finder {
    ...
    try {
      ...
      for (let p of this._settings.paths) {
        ...
        if (stats.isDirectory()) {
          ...
          assert.ok(this.filterToFileResult(p, stats) !== null, startPathDoesNotMatchFindSettings);
        } else {
  ...
  public isTraversableDirPath(dirPath: string): boolean {
    return this.isMatchingDirPathByHidden(dirPath) && this.isMatchingDirPathByOutPatterns(dirPath);
  }
  ...
  public isMatchingArchiveExtension(ext: string): boolean {
    return (
      Finder.emptyOrMatchesAnyString(ext, this._settings.inArchiveExtensions) &&
  ...
  public isMatchingArchiveFileName(fileName: string): boolean {
    return (
      Finder.emptyOrMatchesAnyPattern(fileName, this._settings.inArchiveFilePatterns) &&
  ...
  public isMatchingArchiveFileNameForFilePath(filePath: string): boolean {
    if (
      this._settings.inArchiveFilePatterns.length ||
      this._settings.outArchiveFilePatterns.length
    ) {
  ...
  public isMatchingArchiveFilePath(filePath: string): boolean {
    return (
      this.isMatchingArchiveExtensionForFilePath(filePath) &&
  ...
  public isMatchingExtension(ext: string): boolean {
    return (
      Finder.emptyOrMatchesAnyString(ext, this._settings.inExtensions) &&
  ...
  public isMatchingFileName(fileName: string): boolean {
    return (
      Finder.emptyOrMatchesAnyPattern(fileName, this._settings.inFilePatterns) &&
  ...
  public isMatchingFilePath(filePath: string): boolean {
    return (
      this.isMatchingExtensionForFilePath(filePath) && this.isMatchingFileNameForFilePath(filePath)
    );
  }

  public isMatchingFileType(fileType: FileType): boolean {
    return (
      Finder.emptyOrMatchesAnyFileType(fileType, this._settings.inFileTypes) &&
  ...
  public isMatchingFileSize(fileSize: number): boolean {
    return (
      (this._settings.maxSize === 0 || fileSize <= this._settings.maxSize) &&
  ...
  public isMatchingLastMod(lastMod: number): boolean {
    return (
      (this._settings.maxLastMod === 0 || lastMod <= this._settings.maxLastMod) &&
  ...
  public filterRegularFilePathToFileResult(
    filePath: string,
    fileType: FileType,
  ...
  public filterToFileResult(filePath: string, stat: fs.Stats | null = null): FileResult | null {
    if (
      !this.isNullOrMatchingDirPath(path.dirname(filePath)) ||
  ...
  private async recGetFileResults(
    currentDir: string,
    minDepth: number,
    maxDepth: number,
    currentDepth: number,
    ...
    const filePaths = fs
      .readdirSync(currentDir, { recursive: false })
    ...
    const subDirFileResultArrays = await Promise.all(
      findDirs.map((d) => this.recGetFileResults(d, minDepth, maxDepth, currentDepth + 1)),
    );
  ...
  public async find(): Promise<FileResult[]> {
    ...
    const pathFileResultsArrays = await Promise.all(
      this._settings.paths.map((p) => this.getFileResults(p)),
    );
