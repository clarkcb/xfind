...
export class Finder {
        ...
        try {
            ...
            for (let p of this._settings.paths) {
                ...
                if (stats.isDirectory()) {
                    ...
                    assert.ok(
                        this.filterToFileResult(p, stats) !== null,
    ...
    public isTraversableDirPath(dirPath: string): boolean {
        return (
            this.isMatchingDirPathByHidden(dirPath) && this.isMatchingDirPathByOutPatterns(dirPath)
        );
    }
    ...
    public isMatchingArchiveExtensionForFilePath(filePath: string): boolean {
        if (
            this._settings.inArchiveExtensions.length ||
    ...
    public isMatchingFilePath(filePath: string): boolean {
        return (
            this.isMatchingExtensionForFilePath(filePath) &&
